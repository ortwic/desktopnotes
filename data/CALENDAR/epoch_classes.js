...
!!*/
//======================================//
// Änderungen mit !MOD! gekennzeichnet! //
//======================================//
/**
...
function Epoch(name,mode,targetelement,multiselect) {
	...
	*/
	function calConfig() {
		...
		self.minDate = new Date(self.displayYear,self.displayMonth,1);
		self.maxDate = new Date(self.displayYear+2,self.displayMonth+8,27);
	...
	*/
	function setLang() {
		...
		self.maxrange_caption = 'Maximaler Bereich wurde erreicht.'; // !MOD!
	...
	*/
	function createMainHeading() {
        ...
        addEventHandler(document.getElementById('nextMth'),'click',function(){self.nextMonth();}); // !MOD!
		addEventHandler(document.getElementById('prevMth'),'click',function(){self.prevMonth();}); // !MOD!
	...
	*/
	self.goToMonth = function (year,month) {
		...
		if(testdatemin >= self.minDate && testdatemax <= self.maxDate) {
			// !MOD! self.monthSelect.value = 
            self.displayMonth = month;
			// !MOD! self.yearSelect.value = 
            self.displayYear = year;
            // !MOD!
            if(document.getElementById("selMth")) document.getElementById("selMth").value=shwMonth(month);
            // !MOD!
            if(document.getElementById("selYr")) document.getElementById("selYr").value=year;
	...
	self.selectDates = function (inpdates,selectVal,redraw,removeothers) {
		...
		for(i=0;i<inpdates.length;i++) {
			...
			if(selectVal == true) {
                ...
                inpdates[i].canSelect = true; // !MOD! Bugfix, damit vorselektiertes Datum selber selektiert werden kann
...
// !MOD!
function shwMonth(mth)
{
    mthArray = new Array('Januar','Februar','März','April','Mai','Juni','Juli','August','September','Oktober','November','Dezember');
    return mthArray[mth];
}

// !MOD! Eigene Übergabefunktion
function retVal(myDate)
{
    var days = 0;
    var hrs = (myDate.getTime()-sNow.getTime())/3600000; // Tage in Stunden
    var remHrs = ((ttlArray[3]/60+ttlArray[2])/60+ttlArray[1]); // verbleibende Stunden

    if(hrs > 0) // Wenn gewähltes Datum in gültigem Zeitraum
    {
        days = (hrs - remHrs)/24; // Umrechnung in Tage
        days = Math.ceil(days); // Tage aufrunden
    }
    else
        alert("Ungültiger Zeitraum für gewähltes Datum!");
    
    // Rückgabewert in das Textfeld
...
function setCal(secs, isDay)
{
    var selDate, mSecs;

    if(secs!=""&&isNaN(secs)==false)
    {
        secs=parseInt(secs);
        // Wenn negativer Zahlenwert
        if(secs<0) 
        {
            secs *= (-1);
            document.getElementById("txtDays").value = secs;            
        }
        
        // Wenn Übergabeparameter in Tagen
        if(isDay) 
        {
            ttlArray[0]=secs; // Speichere Tage in Array
            secs=getSecs(secs,ttlArray[1],ttlArray[2],ttlArray[3]);
        }

        mSecs = secs*1000 + sNow.getTime();
        // Wandle Sekunden in gültiges Datum um
        selDate = new Date(mSecs);

        // Selektiere Tag in Kalender
        calTTL.selectDates(new Array(selDate),true,true,true);
        calTTL.goToMonth(selDate.getYear(), selDate.getMonth());
    }
}
